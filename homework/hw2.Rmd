...
Answer: the `sample_mean` is close to 10^10, but the `sample_var_with_question_mark` is far from 1, 
and gets to +/- 10 thousands. The $\theta$s generated each iteration follows the distribution of N(1e10, 1).
But the result calculated by this method is wrong, because the 1e20 part of theta^2 is so large that
it takes up too many "slots" of the fraction of the floating point, so that the rnorm(1) part is rounded
"earlier", resulting in a much bigger or much smaller number. The adding at each iteration can worsen it,
and as iterations go, it goes completely wrong.

The estimates can be extremely large (60000+) or extremely low (-60000).
...
**Ansers:** the slope is expected to be 2, as the error term for the numerical approcimate is $\Delta x^2$ and the analytical estimate is fixed. Therefore, the log relative error is linear to the $log \Delta x$ term.

However, the plot is differet. Only the very beginging part of it agrees to the slope 2 increase. The line starts to plateau after $\Delta x > 2^4$, and increases again and dramatically, after $\Delta x > 2^40$, roughtly. 

That is because the rounding error becomes more significant as $\Delta x$ becomes smaller. In the floating point calculation, the more digits the number needs to be represented, the more it has to be truncated. As the numbers of the digits increase by 1 at a time, the accuary decrease by 2. In log scale, the slope becomes 2$\times$2 = 4 in the later part of the plot.
...
https://github.com/Mavis-Liang/hiperglm

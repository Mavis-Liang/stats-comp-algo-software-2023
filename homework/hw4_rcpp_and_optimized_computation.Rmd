...
**Answers**
An error is raised: "Error: The last inner prod result disagrees with the previous ones."
I don't know why. may be it's related to the storage of the vectors? The vectors are on the edge of exceeding the 
maximum length.
Also, I notice the bug here which is related to the increment of i.
...
**Answers:**
I get $4\times 2$ matrix every time. While the first columns are 1024s, the second column is also same for every rows, but the number varies from time to time.

I guess that's because Rcpp automatically fills the first columns in B with 1s to the length of 1024, but it fills a random number for B's second column. 
...
**Answers:**
When running the first code chunk, R session aborts (even if I run it in the console).

The reason is that when we try to add x to y and x is shorter, we can't allocate space for the numbers to add in y.
But it's ok if y is longer because the space is already allocated. Rcpp adds 0s to y when x is running beyond.

Also, the n in the code which determines how many iterations, are assigned with the length of x.
...
baseR_result <- A %*% B
matmat_result <- row_col_dot_matmat(A, B)
```
...
```{r}
## check and test
matmat_col_ori_result <- col_oriented_matmat(A, B)
...
**Answer**
The colunm-oriented one is actually slower.
The base-R one is much after than the other two.
...
**Answers**
Unfortunately, the MakeVar file on my laptop (Win 11) is not writable. I couldn't find the way to open it as administrator...

But when using default flag (turning on SSE) the Eigen function outperforms the two I wrote, but not as good as base R's %*%.